...
export default function IphoneReveal() {
  ...
  // Imagen: empieza zoom en cámara, se abre al teléfono completo
  const imgScale    = useTransform(scrollYProgress, [0, 0.55], isMobile ? [1, 1] : [1.45, 1])
  const imgY        = useTransform(scrollYProgress, [0, 0.55], isMobile ? ['0%', '0%'] : ['-12%', '0%'])
  const overlayOp   = useTransform(scrollYProgress, [0, 0.4, 0.8], [0.85, 0.55, 0.72])
  ...
  return (
    <div ref={containerRef} style={{ height: isMobile ? '260vh' : '240vh', position: 'relative' }}>
      <div style={{
        ...
        <motion.div style={{
          position: 'absolute', inset: 0,
          background: 'linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0) 30%, rgba(0,0,0,0.82) 100%)',
          opacity: overlayOp,
          pointerEvents: 'none',
        }} />
        ...
        <div style={{
          ...
          <div>
            ...
            <motion.h2 style={{
              ...
              fontSize: isMobile ? 'clamp(72px, 18vw, 110px)' : 'clamp(100px, 11vw, 160px)',
              fontWeight: 200, letterSpacing: isMobile ? -4 : -7,
        ...
        {/* Scroll hint ×" desaparece al empezar */}
        <motion.div style={{
